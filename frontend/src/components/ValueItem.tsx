...
export const ValueItem = ({ index, title, description, icon, subtitle, btnFace, onFocus }: ValueItemProps) => {
  ...
  return (
    <div id={`value-item-${index}`} className={`grid col-span-1 grid-rows-subgrid ${btnFace ? 'row-span-5' : 'row-span-4'} p-6 bg-white shadow-xl shadow-slate-300 hover:shadow-slate-400 transition-all duration-100 rounded-lg rounded-b-3xl pb-2`}>
      <header>
